...
class MainNetwork:
    ...
    def countPrecision(self, testData_x, testData_y):
        ...
        for i in range(dataLen):
            ...
            if input[79]==1: #ja izpildās speciālais nosacījums
                ...
                if Choice==expectedChoice:
                    specialCount+=1
        loss = round(self.loss_function(self.model(testData_x), testData_y), 3)
    ...
    def countPrecision(self, testData_x, testData_y):
        ...
        loss = round(loss_function(self.model(testData_x), testData_y).item(), 3)
        overallPrecision = round(correctCount/dataLen*100, 5)
        print(f'Overll precision: {overallPrecision}; ' + 
              f'loss: {loss}')
    ...
    def calculateLossForOtherData(self):
        self.loss_function = nn.CrossEntropyLoss()
        for j in range(1,3):
            for i in range(2,9):
                locationX = 'Resources/Complexity2/'+str(j)+'-'+str(i)+'/test_x'
                locationY = 'Resources/Complexity2/'+str(j)+'-'+str(i)+'/test_y'
                if path.exists(locationX):
                    data_x = torch.load(locationX)
                    data_y = torch.load(locationY)
                else:
                    data_x, data_y = NN_base.generateSimpleData(i, 100000, locationX, locationY, cardsOnTable=j)
                testPred_y = self.model(data_x)
                loss = self.loss_function(testPred_y, data_y)
                print('cards - '+str(j) + '-' + str(i) + "; loss - " + str(round(loss.item(),3)))
class Experiment22(Bot):
    bot_name = 'Experiment22'
    ...
    def __init__(self, player_name: str):
        ...
        #self.network.countPrecision(torch.load(DataPaths.TEST_X), torch.load(DataPaths.TEST_Y))
    ...
    def handle_game_event(self, event: GameEvent):
        if event.name == EventNames.PartyStartedEvent:
            ...
            if self.hand.size>1 and trick.cards.size>0:
